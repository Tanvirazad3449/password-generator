...
export function createDomElements() {
  ...
  document.body.className = 'bg-slate-300 flex justify-center items-center h-screen flex-col'
...
footer.classList.add('flex', 'justify-center', 'items-center', 'mt-5', 'bg-slate-200', 'p-2', 'rounded');

// Create a link with your name and GitHub icon as the clickable text
const footerLink = document.createElement('a');
footerLink.href = 'https://github.com/TanvirAzad3449';  // Replace with your GitHub username
footerLink.target = '_blank';  // Open in a new tab
footerLink.classList.add('flex', 'items-center', 'space-x-2', 'text-slate-600', 'hover:text-blue-800');

// Create the GitHub icon (SVG)
const img = document.createElement('img');
img.src = githubIcon; // The imported image path
img.alt = 'GitHub Icon'; // Add alt text for accessibility
img.width = 20; // Optional: set image width
img.height = 20; // Optional: set image height


// Create your name text
const nameText = document.createElement('span');
nameText.textContent = 'Tanvir Azad';

// Append icon and name to the link
footerLink.appendChild(img);
footerLink.appendChild(nameText);
footer.appendChild(footerLink)

  return { container, passwordField, generateButton, copyButton, lengthField, upperCaseCheckbox, lowerCaseCheckbox, numbersCheckbox, specialCharsCheckbox, footer };
}
